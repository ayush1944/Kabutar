                ...
                <div className="conversation-area pt-14 pb-10 flex-grow flex flex-col h-full max-h-full overflow-hidden relative">
                    <div ref={messageBox} className="message-box p-1 flex-grow flex flex-col gap-1 overflow-auto max-h-full scrollbar-hide">
                        {/* <div  className="message flex flex-col p-2 bg-slate-50 w-fit rounded-md">
                            <small className='opacity-65 text-xs'>receiver.name</small>
                            <div className='text-sm'>
                                <p>incoming message</p>
                            </div>
                        </div>
                        <div  className="message flex flex-col p-2 ml-[64%] text-nowrap bg-green-200 w-fit rounded-md">
                            <small className='opacity-65 text-xs'>sender.name</small>
                            <div className='text-sm'>
                    ...
                    <div className={`sidePanel w-full h-full flex flex-col gap-2 bg-slate-50 absolute transition-all ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0`}>
                        ...
                        <div className="users flex flex-col gap-2 h-max overflow-auto p-2">
                            {project.users && project.users.map(user => (
                                <div key={user._id} className='user p-2 bg-slate-100 flex gap-2 items-center'>
                                    ...
                                    <div className='flex gap-2 ml-auto'>
                                        <button onClick={() => removeCollaborators(user._id)}   className='p-2 text-slate-700 hover:bg-red-500 hover:text-white rounded-md'><i className="ri-delete-bin-2-fill"></i></button>
                                    </div>
                                </div>
                            ))}
                        </div>
